...
| `http/` | Маршруты локального сервера, проверка loopback и заголовка `x-lapka` на всём, что меняет машину | `startServer({port, webDir, ctx})`; `/api/look`, `/api/resolve`, `/api/dubs`, `/api/stream/*`, `/api/save*`, `/api/saves*`, `/api/library*`, `/api/home*`, `/api/cache*`, `/api/state*` | готов |
