...
| `session/` | Сессия: как Lapka ходит в сеть | `fetch(url, {referer, headers}) → {status, headers, body}`; провайдеры кук по браузерам `cookiesFor(domain)` | шов |
| `discover/` | Разбор страницы в отчёт: сериал, серии, плееры, озвучки, связи | `discover(html, url, {profile}) → Report`, `reportToContribution(report)` | шов |
...
| `http/` | Маршруты локального сервера, проверка loopback | | шов |
...
Браузерная часть живёт в `web/` (появится при нарезке `app.js`).
