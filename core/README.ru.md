...
| `store/` | Папка Lapka: кэш по потокам с лимитом и вытеснением; библиотека как сайдкары на диске; состояние; `config.mjs` помнит выбранную папку в системной папке настроек (`systemConfigDir()`, разовый перенос из старого `.dev/config.json`; `LAPKA_CONFIG` называет файл), `POST /api/home?path=` создаёт и переключает её на ходу | `openStore({home})` → `cache.*`; `openLibrary(home)` → `placeFor`, `keepSeries`, `list`, `GET /api/library`, `/api/library/file`; `openState(own)` → позиции (одна на серию, независимо от озвучки), выбор озвучки, настройки, вставленные ссылки с обложками файлами (`history`, `remember`, `lastStop`, `/api/history*`), `/api/state*` | готово; знание — шов. Пока папка не выбрана, это `.dev/home` в проекте (клон, архив) или `Lapka` в домашней папке (Lapka из кэша npm: npx, глобальная установка), `LAPKA_HOME` переопределяет |
...
`main.mjs` — точка входа `npm start`, лончеров и `npx`; занятый порт называется словами, а не
трассой: другой Lapka там отдаётся браузер, другая программа значит другой порт.
