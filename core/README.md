...
| `store/` | The Lapka folder: a cache per stream with a limit and eviction; the library as sidecars on disk; the state; `config.mjs` remembers the chosen folder in `.dev/config.json`, `POST /api/home?path=` creates and switches it on the fly | `openStore({home})` → `cache.*`; `openLibrary(home)` → `placeFor`, `keepSeries`, `list`, `remove`, `GET /api/library`, `/api/library/file`; `openState(own)` → positions, the dub choice, settings, the save records, `/api/state*` | ready; knowledge is a seam. The folder during development is `.dev/home` in the project, or `LAPKA_HOME`; the user's choice comes from the UI |
...
| `http/` | The routes of the local server, the loopback check and the `x-lapka` header on everything that changes the machine | `startServer({port, webDir, ctx})`; `/api/look`, `/api/resolve`, `/api/dubs`, `/api/stream/*`, `/api/save*`, `/api/saves*`, `/api/library*`, `/api/home*`, `/api/cache*`, `/api/state*` | ready |
