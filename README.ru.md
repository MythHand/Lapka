...
`./stop.sh`, либо кнопка «Выключить Lapka» внизу настроек (шестерёнка). Обновить:
`update.command`, `update.bat` или `./update.sh` подтягивает новую версию в ту же папку
(не второй `git clone`: git не пишет в существующую папку) и перезапускает Lapka,
если она работала.
...
- `npm run dev` — Lapka на 8800 и синтетический сайт для тестов на 8801 (сайт собирает клипы через ffmpeg; без него поднимется только Lapka).
